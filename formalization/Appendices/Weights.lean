/-
Main text, §"A simplex-valued map on bilinear forms".
...
*weights* of `L` (for `‖L‖ = 1`) are

    a = ‖T‖²,  b = 2⟨α, β⟩,  c = ‖(α-β)₊‖²,  d = ‖(α-β)₋‖²,

where `x₊ = max(x,0)` and `x₋ = max(-x,0)` entrywise.  The map
`w = (a,b,c,d)` is extended to all `L ≠ 0` invariantly under positive
scaling; we build the normalization `‖L‖² = 1` into the definitions by
dividing by `‖L‖²`.
...
`(max(-λ_i, 0))_i`.  The weights do not depend on the amount of zero
padding.
-/
...
/-! ### The lists `α` and `β` and the weights -/
...
/-- The weight `a = ‖T‖²/‖L‖²`. -/
noncomputable def weightA (L : Matrix (Fin N) (Fin N) ℝ) : ℝ :=
  frobSq (skewPart L) / frobSq L

/-- The weight `b = 2⟨α, β⟩/‖L‖²`, where `α`, `β` are the lists of the
symmetric part of `L`. -/
noncomputable def weightB (L : Matrix (Fin N) (Fin N) ℝ) : ℝ :=
  2 * (∑ j, alphaList (symPart L) (symPart_isHermitian L) j *
    betaList (symPart L) (symPart_isHermitian L) j) / frobSq L

/-- The weight `c = ‖(α-β)₊‖²/‖L‖²`. -/
noncomputable def weightC (L : Matrix (Fin N) (Fin N) ℝ) : ℝ :=
  (∑ j, max (alphaList (symPart L) (symPart_isHermitian L) j -
    betaList (symPart L) (symPart_isHermitian L) j) 0 ^ 2) / frobSq L

/-- The weight `d = ‖(α-β)₋‖²/‖L‖²`. -/
noncomputable def weightD (L : Matrix (Fin N) (Fin N) ℝ) : ℝ :=
