\href{https://aodesky.github.io/attention-bilinear-forms/}{Project home}
\quad $\cdot$ \quad
\href{dep_graph_document.html}{Dependency graph}

This blueprint presents the mathematical results implemented in the
audited Lean formalization of \emph{On attention heads and bilinear forms}.
It covers Appendices A, B and C, together with Definition 6 and Theorem 5
from the main text. Lemma, corollary, proposition and theorem numbers
match the paper; auxiliary definitions are named separately.

Each result includes a proof outline and links to its Lean declarations.
The dependency graph records the mathematical dependencies between these
results and definitions; internal helper lemmas and mathlib dependencies
are omitted. A green result and proof indicate that both have been
formalized. The source links point to the audited revision of the Lean files.

The empirical observations and the other main-text theorems are outside
this formalization. In particular, Lemma 7 is formalized here, but the
main-text accumulation theorem that applies it is not. The two expository
remarks following Lemma 11 are also outside the scope.

\chapter{Bilinear forms and their shapes}
...
\begin{definition}[Shape of a bilinear form]
\label{def:shape}
\lean{Appendices.sortDesc, Appendices.alphaList, Appendices.betaList, Appendices.shapeA, Appendices.shapeB, Appendices.shapeC, Appendices.shapeD}
\leanok
\uses{def:parts}
...
padded with zeros to equal length. Define $s(L)=(a,b,c,d)$ by
\[
 a=\frac{\norm{T}^2}{\norm{L}^2},\qquad
...
$\E\tr(L^2)=nN\sigma_K^2\sigma_Q^2$. Sum the entries and use Lemma 6.
\end{proof}
...
Lemma 6 gives the result. The denominator is nonzero almost surely.
...
\begin{theorem}[Fibers of the shape map over the simplex faces]
\label{thm:faces}
\lean{Appendices.shapeA_nonneg, Appendices.shapeB_nonneg, Appendices.shapeC_nonneg, Appendices.shapeD_nonneg, Appendices.shape_sum_eq_one, Appendices.shapeA_eq_zero_iff, Appendices.shapeA_eq_one_iff, Appendices.shapeB_eq_zero_iff, Appendices.shapeB_eq_one_iff, Appendices.shapeD_eq_zero_iff_forall, Appendices.shapeD_eq_zero_iff_decomp, Appendices.shapeD_eq_zero_decomp_commute, Appendices.shapeD_eq_one_iff, Appendices.shapeC_eq_zero_iff_forall, Appendices.shapeC_eq_zero_iff_decomp, Appendices.shapeC_eq_zero_decomp_commute, Appendices.shapeC_eq_one_iff}
\leanok
\uses{def:shape}
Let $L\ne0$ and $s(L)=(a,b,c,d)$. The coordinates are nonnegative
